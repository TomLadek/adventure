...
const cmsControlsStore = useCmsControlsStore(),
      ...
      selectedText = ref(""),
      editorBasicExtensions = [ // Extensions we want always
        Placeholder.configure({
            placeholder: props.emptyPlaceholder || "Empty"
        })
      ],
      editorAllExtensions = [...editorBasicExtensions, ...(props.headlineToolsOnlyEditor ? [ // If headline tools only -> minimal extensions
        Document,
        Text,
        Paragraph,
        History
      ] : [ // Not headline tools -> all extensions
        StarterKit, // includes Document, Text, Paragraph, History and more
        Link.configure({
            openOnClick: false,
            HTMLAttributes: {
              rel: "noreferrer",
      ...
      editor = useEditor({
        extensions: editorAllExtensions,
  ...
  <div v-else class="cms-text-editor-container" :class="class">
    ...
    <div class="cms-text-editor-controls" ref="cmsTextEditorControls" style="display: none;">
      <ul class="editor-actions-container">
        <li v-if="!headlineToolsOnlyEditor">
